...
exports.getMyRequests = function(person_id, callback) {
  ...
  var getAll = flow.define(
    function(db_model, person_id, callback) {
      this.db_model = db_model;
      this.person_id = person_id;
      this.callback = callback;
      this.db_model.Person.get(this.person_id, this);
    },
    function(err, person) {
      if (err) {
        this.callback({err: err});
      } else if (person.system_admin) {
        this.db_model.Request.find('description', this);
      } else {
        this.db_model.BandMember.find({ person_id: this.person_id, band_admin: true }, function(err, band_members) {
          if (err) {
            this.callback({err: err});
          } else {
            var args;
            ...
            this.db_model.Request.find(args, 'description', this);
          }
        }.bind(this));
      }
    },
  ...
  db_orm.connect(function(db_model) {
    getAll(db_model, person_id, callback);
  });
};
