...
BandMember.prototype.updateByPersonAndBandId = function(person_id, band_id, params, callback) {
  this.getByPersonAndBandId(person_id, band_id, function(result) {
    if (result.err) {
      callback(result);
    } else {
      result.band_member.band_admin = params.band_admin;
      this.update(result.band_member, callback);
    }
  }.bind(this));
};
