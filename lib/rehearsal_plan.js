...
exports.save = function(band_id, rehearsal_date, run_through, learning, cb) {
  db_orm.RehearsalPlan.create([{band_id: band_id, rehearsal_date: rehearsal_date}], function(err, rows) {
    if (err) {
      cb(err);
    } else {
      var sequence = 0;
...
exports.getPlanSelect_ = function(plan_or_plans) {
  if (Array.isArray(plan_or_plans)) {
    id_list = plan_or_plans.map(function(plan) {
      return plan.id;
    });
    return {rehearsal_plan_id: id_list};
  } else {
    return {rehearsal_plan_id: plan_or_plans.id};
  }
};

exports.getRunThroughSongs_ = function(plan_or_plans) {
debugger;//XXX
  return Q.promise(function(resolve, reject, notify) {
debugger;//XXX
    db_orm.RehearsalRunThrough.find(
      this.getPlanSelect_(plan_or_plans),
      ['rehearsal_plan_id', 'sequence'],
      function(err, run_through_songs) {
debugger;//XXX
        if (err) return reject(err);
        return resolve(run_through_songs);
      }
    );
  }.bind(this));
};

exports.getLearningSongs_ = function(plan_or_plans) {
debugger;//XXX
  return Q.promise(function(resolve, reject, notify) {
debugger;//XXX
    db_orm.RehearsalLearning.find(
      this.getPlanSelect_(plan_or_plans),
      ['rehearsal_plan_id', 'sequence'],
      function(err, learning_songs) {
debugger;//XXX
        if (err) return reject(err);
        return resolve(learning_songs);
      }
    );
  }.bind(this));
};

exports.getForBand = function(band_id, cb) {
  db_orm.RehearsalPlan.find({band_id: band_id}, function(err, rows) {
    if (err) {
      cb(err);
    } else {
debugger;//XXX
      Q.all([
        this.getRunThroughSongs_(rows),
        this.getLearningSongs_(rows)
      ]).then(function(plan_items) {
debugger;//XXX
        cb(null, {
          all_rehearsal_plans: rows,
          run_through_songs: plan_items[0],
          learning_songs: plan_items[1]
        });
...
debugger;//XXX
      Q.all([
        this.getRunThroughSongs_(plan),
        this.getLearningSongs_(plan)
      ]).then(function(plan_items) {
debugger;//XXX
        cb(null, {
          rehearsal_plan: plan,
