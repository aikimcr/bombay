...
exports.save = function(band_id, rehearsal_date, run_through, learning, cb) {
  db_orm.RehearsalPlan.create([{band_id: band_id, rehearsal_date: rehearsal_date}], function(err, rows) {
    if (err) {
      ...
      db_orm.RehearsalPlanRunThroughSong.create(rt_songs, function(err, rows) {
        if (err) {
          cb(err);
        } else {
          sequence = 0;
          ...
          db_orm.RehearsalPlanLearningSong.create(learning_songs, function(err, rows) {
            if (err) {
              cb(err);
            } else {
              cb(null, plan_id);
...
exports.getRunThroughSongs = function(plan, cb) {
  db_orm.RehearsalPlanRunThroughSong.find(
...
exports.getLearningSongs = function(plan, cb) {
  db_orm.RehearsalPlanLearningSong.find(
    {rehearsal_plan_id: plan.id},
    ['rehearsal_plan_id', 'sequence'],
    cb
  );
...
exports.get = function(plan_id, cb) {
  db_orm.RehearsalPlan.get(plan_id, function(err, plan) {
    if (err) {
      ...
      this.getRunThroughSongs(plan, function(err, run_through_songs) {
        if (err) {
          cb(err);
        } else {
          this.getLearningSongs(plan, function(err, learning_songs) {
            if (err) {
              cb(err);
            } else {
              cb(null, {
                rehearsal_plan: plan,
                rehearsal_plan_run_through_songs: run_through_songs,
                rehearsal_plan_learning_songs: learning_songs
              });
            }
          }.bind(this));
        }
